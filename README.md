...
**Zero-cooperation session recovery after gateway restart.**
...
A shell script runs on every gateway start (systemd `ExecStartPost`):
...
一个 shell 脚本在每次网关启动时自动运行（systemd `ExecStartPost`）：
